...
Phage::vec2::vec2(float xi, float yi)
{
	x = xi;
	y = yi;
}
...
Phage::vec3::vec3(float xi, float yi, float zi)
{
	x = xi;
	y = yi;
	z = zi;
...
int Phage::ray::CreateRayFromTwoPoints(glm::vec3 p1, glm::vec3 p2)
{
	origin = p1;
	direction = p2-p1;
...
float Phage::ray::CalculateYFromX(float x)
{
...
glm::vec3 Phage::ray::CalculatePointFromT(float t)
{
	glm::vec3 returnVec;
...
int Phage::ray::CalculateIntersectTriangle(glm::vec3 v0, glm::vec3 v1, glm::vec3 v2, glm::vec4* out)
{
	glm::vec3 e1 = v1 - v0;
	glm::vec3 e2 = v2 - v0;
